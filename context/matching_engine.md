...
- **Location Compatibility**: `FIND_IN_SET(A.location, B.pref_location)` AND `FIND_IN_SET(B.location, A.pref_location)`.
- **Religious Alignment**: Mutual match between `religion` and `pref_religion`.
- **Modesty Alignment**: Mutual match between `modesty` and `pref_modesty`.
...
## 5. Candidate Generation Limits & Quotas
